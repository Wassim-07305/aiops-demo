...
type SourceItem = { id: string; question: string };
  ...
  async function onSend() {
    ...
    if (activeTab === 'Support') {
      try {
        ...
        const reply = data?.ok && typeof data.reply === 'string' ? data.reply : '⚠️ Erreur serveur (support)';

        setLastSupportReply(reply);
        const srcList: SourceItem[] = Array.isArray(data?.sources) ? data.sources as SourceItem[] : [];
        setLastSupportSources(srcList.map((s) => s.question));
        ...
        if (data?.needHandoff) {
          setMessages((m) => [
            ...m,
            {
              id: uid(),
              role: 'agent',
              text: 'Je peux vous mettre en relation avec un humain tout de suite. Cliquez sur Handoff.',
      ...
      setMessages((m) => [
        ...m,
        {
          ...
          role: 'agent',
          text:
            "Pour la pré-vente, utilise le formulaire au-dessus (besoin, budget, délai, email) puis clique Envoyer. Je te donne ensuite un lien Calendly 👍",
          at: new Date().toISOString(),
        },
      ]);
      return;
    ...
    <div className="min-h-screen bg-background text-foreground">
      <div className="mx-auto max-w-4xl p-6 sm:p-10">
        <header className="mb-4 sm:mb-6">
          ...
          <p className="text-sm text-foreground/60">Squelette — 2 onglets, chat &amp; handoff</p>
        </header>
        ...
        <div className="rounded-xl border border-foreground/15 bg-background">
            ...
            <form
              ...
              onSubmit={async (e) => {
                ...
                try {
                  ...
                  let data: { ok?: boolean; score?: number; calendlyUrl?: string } | null = null;
                  ...
                  if (data?.ok) {
                    ...
                    setTool('');
                    setEmail('');
                    // Option: window.open(data.calendlyUrl!, '_blank');
                  } else {
                    setMessages((m) => [
                      ...m,
          ...
          {/* Chat area */}
          <div className="h-[420px] overflow-auto p-4 space-y-2" aria-live="polite" role="log">
            {messages.map((m) => (
              <div
                key={m.id}
                className={[
            ...
            {messages.length === 0 && <p className="text-sm text-foreground/60">Aucun message pour l’instant…</p>}
          </div>
